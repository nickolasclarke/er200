...
def find_total_energy(obj):
    total_energy = 0
    for key, value in obj.items():
        value['energy_day'] = pot_P_wind(rho, 1, value['mean']) * 60 * (24 * value['freq'])
...
print('total energy per day:', format(total_energy, '.2g'), 'kwh \n' +
    'average power per day:', format(avg_power, '.2g'), 'kw')

```

**ANSWERS:**  

total energy per day: 6.1e+02 kwh
average power per day: 25 kw
...
e_kwh_m2 = 15
eff = 0.38
e_output_mw = e_kwh_m2 * vestas['swept_area_m2'] * 5 * eff / 1e3

print(format(e_output_mw, '.2g'), 'MW')
...
```python
v10_kwh_m2 = pot_P_wind(rho, 1, 10) * 60 * 24 / 1e3
v10_kwh_m2 / total_energy
```
